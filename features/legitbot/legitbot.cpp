...
void legit_bot::do_smooth(const Vector& currentAngle, const Vector& aimAngle, Vector& angle)
{
	...
	aim_punch = globals.local()->m_aimPunchAngle() * recoil_scale;
	aim_punch *= (g_cfg.legitbot.weapon[globals.g.current_weapon].rcs / 100.f);

	Vector delta = aimAngle - (currentAngle + aim_punch);
	math::normalize_angles(delta);

	if (g_cfg.legitbot.weapon[globals.g.current_weapon].smooth_type == 1) {
...
void legit_bot::legit_resolver(player_t* entity)
{
...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (globals.g.weapon->is_non_aim() || globals.g.weapon->m_iClip1() <= 0)
	{
		target = nullptr;
		point = Vector(0, 0, 0);
		started_scope = false;
	...
	if (globals.g.weapon->is_sniper() && started_unscope && globals.g.weapon->can_fire(false) && globals.local()->m_bIsScoped())
	{
		scope_delay = 0;
		cmd->m_buttons |= IN_ATTACK2;

		if (globals.g.weapon->m_zoomLevel() == 3)
			started_unscope = false;
	...
	if (target_switch_delay > m_globals()->m_realtime)
	{
		...
		if ((key_binds::get().get_key_bind_state(1) || key_binds::get().get_key_bind_state(0)) && !g_cfg.legitbot.weapon[globals.g.current_weapon].rcs_type)
			do_rcs(cmd);
	...
	auto is_silent = !globals.local()->m_iShotsFired() && g_cfg.legitbot.weapon[globals.g.current_weapon].silent_fov;
	point = Vector(0, 0, 0);
	...
	if (point == Vector(0, 0, 0))
	{
		...
		if (is_silent)
		{
			find_best_point(cmd, g_cfg.legitbot.weapon[globals.g.current_weapon].silent_fov);
			if (point == Vector(0, 0, 0))
				return;
		}
		else
	...
	// is enemy in smoke?
	if (!g_cfg.legitbot.do_if_enemy_in_smoke && in_smoke(globals.g.eye_pos, target->hitbox_position(0)))
	{
		autofire_delay = 0;
	...
	// are we flashed?
	if (!g_cfg.legitbot.do_if_local_flashed && globals.local()->m_flFlashDuration() >= 0.1f)
	{
		autofire_delay = 0;
	...
	// is local in air?
	if (!g_cfg.legitbot.do_if_local_in_air && !(globals.local()->m_fFlags() & FL_ONGROUND))
	{
		if (should_do_rcs)
			do_rcs(cmd);
	...
	if (!globals.local()->m_iShotsFired() && key_binds::get().get_key_bind_state(0) && autofire_delay < g_cfg.legitbot.autofire_delay)
	{
		autofire_delay += 1;
		return;
	}
	else if (!globals.local()->m_iShotsFired())
		autofire_delay = 0;

	auto weapon = globals.g.weapon;


	if (g_cfg.legitbot.autoscope && (!globals.local()->m_bIsScoped() || (started_scope && scope_delay > 9)) && weapon->is_sniper())
	{
		cmd->m_buttons |= IN_ATTACK2;
	...
	if (weapon->is_sniper() &&
		g_cfg.legitbot.sniper_in_zoom_only && !g_cfg.legitbot.autoscope && !globals.local()->m_bIsScoped())
	{
		if (should_do_rcs)
			do_rcs(cmd);
	...
	auto is_silent_s = !globals.local()->m_iShotsFired() && g_cfg.legitbot.weapon[globals.g.current_weapon].silent_fov && target_fov <= g_cfg.legitbot.weapon[globals.g.current_weapon].silent_fov;

	if (g_cfg.legitbot.weapon[globals.g.current_weapon].auto_stop)
		cmd->m_forwardmove = cmd->m_sidemove = 0;

	aim_angle = math::calculate_angle(globals.g.eye_pos, point).Clamp();
	...
	if (g_cfg.legitbot.weapon[globals.g.current_weapon].rcs_type || should_do_rcs)
		do_rcs(cmd);

	auto hitchanced = hitchance(target, aim_angle, point, targets[target->EntIndex()].hitbox) >= g_cfg.legitbot.weapon[globals.g.current_weapon].autofire_hitchance;

	if (key_binds::get().get_key_bind_state(0) && globals.g.weapon->can_fire(true) && hitchanced)
		cmd->m_buttons |= IN_ATTACK;

	if (cmd->m_buttons & IN_ATTACK && !started_unscope)
		if (globals.g.weapon->is_sniper() && g_cfg.legitbot.unscope)
			started_unscope = true;
}

void legit_bot::calc_fov()
{
	if (globals.local()->m_iShotsFired() < 2)
		fov_t = g_cfg.legitbot.weapon[globals.g.current_weapon].fov;

	else if (globals.local()->m_iShotsFired() >= 2 && g_cfg.legitbot.weapon[globals.g.current_weapon].custom_rcs_fov > 0.f)
		fov_t = g_cfg.legitbot.weapon[globals.g.current_weapon].custom_rcs_fov;
...
{
	if (globals.local()->m_iShotsFired() < 2)
		smooth_t = g_cfg.legitbot.weapon[globals.g.current_weapon].smooth;

	else if (globals.local()->m_iShotsFired() >= 2 && g_cfg.legitbot.weapon[globals.g.current_weapon].custom_rcs_smooth >= 1.f)
		smooth_t = g_cfg.legitbot.weapon[globals.g.current_weapon].custom_rcs_smooth;
...
void legit_bot::do_rcs(CUserCmd* cmd)
{
	...
	if (weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER
		|| g_cfg.legitbot.weapon[globals.g.current_weapon].rcs <= 0
		|| weapon->is_non_aim()
		|| globals.local()->m_iShotsFired() < 2)
	{
...
void legit_bot::find_best_point(CUserCmd* cmd, float fov_v)
{
	...
	if (g_cfg.legitbot.weapon[globals.g.current_weapon].priority == 0)
	{
		for (auto bone : { HITBOX_HEAD, HITBOX_PELVIS, HITBOX_CHEST, HITBOX_UPPER_CHEST, HITBOX_STOMACH })
		{
			...
			math::vector_angles(bone_pos - globals.g.eye_pos, ang);
			math::normalize_angles(ang);
			distance = globals.g.eye_pos.DistTo(bone_pos);

			if (g_cfg.legitbot.weapon[globals.g.current_weapon].fov_type == 1)
			...
			auto fire_data = autowall::get().wall_penetration(globals.g.eye_pos, bone_pos, target);

			if (!fire_data.visible && !g_cfg.legitbot.weapon[globals.g.current_weapon].awall_dmg)
				continue;

			if (!fire_data.visible && g_cfg.legitbot.weapon[globals.g.current_weapon].awall_dmg > 0 && fire_data.damage < g_cfg.legitbot.weapon[globals.g.current_weapon].awall_dmg)
				continue;
	...
	else if (g_cfg.legitbot.weapon[globals.g.current_weapon].priority == 1) // head only
	{
		float distance = 0.f;
		float fov = FLT_MAX;
		Vector ang = Vector(0, 0, 0);
	...
	else if (g_cfg.legitbot.weapon[globals.g.current_weapon].priority == 2) // nearest body
	{
...
void legit_bot::find_target()
{
	...
	for (auto i = 1; i < m_globals()->m_maxclients; i++)
	{
		...
		if (!g_cfg.legitbot.friendly_fire && e->m_iTeamNum() == globals.local()->m_iTeamNum())
			continue;

		Vector engine_angles;
		m_engine()->GetViewAngles(engine_angles);
		targets[i].fov = math::get_fov(engine_angles, math::calculate_angle(globals.g.eye_pos, e->hitbox_position(HITBOX_CHEST)));
...
int legit_bot::hitchance(player_t* target, const Vector& aim_angle, const Vector& point, int hitbox) //-V688
{
	...
	if ((globals.g.eye_pos - point).Length() > weapon_info->flRange)
		return final_hitchance;
	...
	legit_bot::get().legit_resolver(target);
	...
	auto is_special_weapon = globals.g.weapon->m_iItemDefinitionIndex() == WEAPON_AWP || globals.g.weapon->m_iItemDefinitionIndex() == WEAPON_G3SG1 || globals.g.weapon->m_iItemDefinitionIndex() == WEAPON_SCAR20 || globals.g.weapon->m_iItemDefinitionIndex() == WEAPON_SSG08;
	auto inaccuracy = weapon_info->flInaccuracyStand;

	if (globals.local()->m_fFlags() & FL_DUCKING)
	{
	...
	if (globals.g.inaccuracy - 0.000001f < inaccuracy)
		final_hitchance = 101;
	else
	{
		...
		for (auto i = 0; i < 256; ++i)
		{
			auto inaccuracy = spread_values[i][0] * globals.g.inaccuracy;
			auto spread = spread_values[i][1] * globals.g.spread;
			...
			auto end = globals.g.eye_pos + direction * weapon_info->flRange;

			if (hitbox_intersection(target, target->m_CachedBoneData().Base(), hitbox, globals.g.eye_pos, end))
				++hits;
		}
