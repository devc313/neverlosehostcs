...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.legitbot.legit_resolver, false, crypt_str("Legit Resolver"));
	for (auto i = 0; i < 6; i++)
	{
	...
	setup_item(&g_cfg.ragebot.yaw_resolve, false, crypt_str("Ragebot.yaw_resolve"));
	...
	setup_item(&g_cfg.misc.forceRelayCluster, 0, crypt_str("Misc.forceRelayCluster"));
	...
	setup_item(&g_cfg.misc.anti_exploit, false, crypt_str("anti-exploit"));
...
{
	std::string file = crypt_str("C:\\NeverloseHost\\Configs\\CSGO\\") + config;
	json allJson;
...
{
	std::string file = crypt_str("C:\\NeverloseHost\\Configs\\CSGO\\");

	files.clear();

	std::string path = file + crypt_str("/*.es");
	WIN32_FIND_DATA fd;
