...
void c_menu::menu_setup(ImGuiStyle& style) //-V688
{
	...
	D3DXCreateTextureFromFileInMemoryEx(device, &logo, sizeof(logo), 9999, 9999, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &texture);
...
void load_config()
{
	...
	eventlogs::get().add(crypt_str("Loaded ") + files.at(g_cfg.selected_config) + crypt_str(" config"), Color(255, 255, 255), false);
}
...
void save_config()
{
	...
	cfg_manager->config_files();
	eventlogs::get().add(crypt_str("Saved ") + files.at(g_cfg.selected_config) + crypt_str(" config"), Color(255, 255, 255), false);
}
...
void remove_config()
{
	...
	eventlogs::get().add(crypt_str("Removed ") + files.at(g_cfg.selected_config) + crypt_str(" config"), Color(255, 255, 255), false);
...
void add_config()
{
	...
	eventlogs::get().add(crypt_str("Added ") + g_cfg.new_config_name + crypt_str(" config"), Color(255,255,255) ,false);

	if (g_cfg.new_config_name.find(crypt_str(".es")) == std::string::npos)
		g_cfg.new_config_name += crypt_str(".es");
...
{
	std::string file_path = crypt_str("C:\\NeverloseHost\\Scripts\\CSGO\\");
...
void c_menu::draw_antiaim(int child)
{
	...
	{
		tab_start();
		ImGui::BeginChild(crypt_str("##AA1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
	...
	else
	{
		...
		ImGui::BeginChild(crypt_str("##AA1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			static auto sv_maxusrcmdprocessticks = m_cvar()->FindVar("sv_maxusrcmdprocessticks"); // по дефолту 16, но надо делать 14 тк 2 тика уйдут на резерв
			...
			ImGui::Checkbox(crypt_str("Fake lag"), &g_cfg.antiaim.fakelag);
				...
				ImGui::SliderInt(crypt_str("Limit"), &g_cfg.antiaim.fakelag_amount, 1, sv_maxusrcmdprocessticks->GetInt() - 2);
				...
				if (enabled_fakelag_triggers)
					ImGui::SliderInt(crypt_str("Triggers limit"), &g_cfg.antiaim.triggers_fakelag_amount, 1, sv_maxusrcmdprocessticks->GetInt() - 2);
		}
		ImGui::EndChild();
		...
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				if (g_cfg.ragebot.double_tap)
				{
					...
					ImGui::Checkbox(crypt_str("DT Instant"), &g_cfg.ragebot.slow_teleport);
				}
	...
	else
	{
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				{
					ImGui::Checkbox(crypt_str("DT HitChance"), &g_cfg.ragebot.weapon[hooks::rage_weapon].double_tap_hitchance);

					if (g_cfg.ragebot.weapon[hooks::rage_weapon].double_tap_hitchance)
						ImGui::SliderInt(crypt_str("DT HitChance Amount"), &g_cfg.ragebot.weapon[hooks::rage_weapon].double_tap_hitchance_amount, 1, 100);
				}
		...
		ImGui::BeginChild(crypt_str("##SKINCHANGER__TAB"), ImVec2(686 * dpi_scale, child_height * dpi_scale)); // first functional child
		{
			...
			ImGui::BeginChild(crypt_str("##SKINCHANGER__CHILD"), ImVec2(686 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				else
				{
					...
					if (current_profile == 1)
					{
						for (auto i = 1; i < main_kits.size(); ++i)
						{
	...
	if (!child)
	{
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Hotkey list"), &g_cfg.misc.keybinds);
	...
	else
	{
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				draw_combo(crypt_str("Force region"), g_cfg.misc.forceRelayCluster, relays, ARRAYSIZE(relays));
			}
...
void c_menu::draw_settings(int child)
{
	...
	ImGui::BeginChild(crypt_str("##CONFIGS__FIRST"), ImVec2(686 * dpi_scale, (child_height - 35) * dpi_scale));
	{
		...
		{
			std::string folder = crypt_str("C:\\NeverloseHost\\Configs\\CSGO\\");
...
void c_menu::draw_lua(int child)
{
	if (!child)
	{
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			{
				std::string folder = crypt_str("C:\\NeverloseHost\\Scripts\\CSGO\\");
				ShellExecute(NULL, crypt_str("open"), folder.c_str(), NULL, NULL, SW_SHOWNORMAL);
			}
			...
			if (ImGui::CustomButton(crypt_str("Load script"), crypt_str("##SCRIPTS__LOAD"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
			{
				...
				eventlogs::get().add(crypt_str("Loaded ") + scripts.at(selected_script) + crypt_str(" script"), Color(255, 255, 255),false); //-V106
			}
			...
			if (ImGui::CustomButton(crypt_str("Unload script"), crypt_str("##SCRIPTS__UNLOAD"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
			{
				...
				eventlogs::get().add(crypt_str("Unloaded ") + scripts.at(selected_script) + crypt_str(" script"), Color(255, 255, 255), false); //-V106
			}
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str(""), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
	{
		auto p = ImGui::GetWindowPos();
		auto draw = ImGui::GetWindowDrawList();

		draw->AddRectFilled(p, ImVec2(p.x + (150 * dpi_scale), p.y + (550 * dpi_scale)), ImColor(16, 16, 16, 255), 3.0f, ImDrawCornerFlags_All);
		draw->AddRectFilled(ImVec2(p.x + (900 * dpi_scale), 0), ImVec2(p.x + (160 * dpi_scale), p.y + (550 * dpi_scale)), ImColor(16, 16, 16, 255), 3.0f, ImDrawCornerFlags_None);
		...
		ImGui::SetCursorPos(ImVec2(-120 * dpi_scale, -170 * dpi_scale));
		ImGui::Image(texture, ImVec2(350, 250));
		...
		{																			 // KEYWORD : TABS_FIND
			...
			ImGui::SetCursorPos(ImVec2(13 * dpi_scale, 80 * dpi_scale));

			ImGui::Text(""); // ugly as fuck
			ImGui::PushFont(c_menu::get().futura);
			const char* strTabsNames[] = { crypt_str("RAGE"), crypt_str("ANTI-AIM"), crypt_str("LEGIT"), crypt_str("VISUALS"), crypt_str("PLAYERS"), crypt_str("MISC"), crypt_str("SETTINGS"), crypt_str("SCRIPTS")};
			for (int iTab = 0; iTab < IM_ARRAYSIZE(strTabsNames); iTab++)
				if (ImGui::TabButton(strTabsNames[iTab], active_tab == iTab)) active_tab = iTab;
			ImGui::PopFont();
			ImGui::PopStyleVar();
		}
		ImGui::EndChild();
		...
		{																			   // KEYWORD : FUNC_FIND
			...
			case 3: draw_tabs_visuals(); break;
			case 4: draw_tabs_players(); break;
			case 5: draw_tabs_misc(); break;
			default: break;
			}
			...
			ImGui::BeginGroup();  // groupping two childs to prevent multipadding
			{
				...
				if (last_tab == 3 && visuals_section == 2)
				{
					draw_visuals(-1);
				}
				else if (last_tab == 6)
				...
				else
				{
					ImGui::BeginChild(crypt_str("##content_first"), ImVec2(339 * dpi_scale, child_height * dpi_scale)); // first functional child
					{
						...
						case 2: draw_legit(0); break;
						case 3: draw_visuals(0); break;
						case 4: draw_players(0); break;
						case 5: draw_misc(0); break;
					...
					ImGui::BeginChild(crypt_str("##content_second"), ImVec2(339 * dpi_scale, child_height * dpi_scale)); // second functional child
					{
						...
						case 2: draw_legit(1); break;
						case 3: draw_visuals(1); break;
						case 4: draw_players(1); break;
						case 5: draw_misc(1); break;
