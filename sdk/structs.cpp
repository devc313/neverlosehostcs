...
#include "..\features\lagcompensation\setup_bones.h"

datamap_t* entity_t::GetPredDescMap()
{
	typedef datamap_t* (__thiscall* Fn)(void*);
	return call_virtual< Fn >(this, globals.indexes.at(16))(this);
}
...
void entity_t::set_model_index(int index)
{
	...
	return call_virtual<Fn>(this, globals.indexes.at(7))(this, index);
}
...
float weapon_t::get_inaccuracy()
{
	...
	return call_virtual<float(__thiscall*)(void*)>(this, globals.indexes.at(9))(this);
}
...
float weapon_t::get_spread()
{
	...
	return call_virtual<float(__thiscall*)(void*)>(this, globals.indexes.at(10))(this);
}
...
void weapon_t::update_accuracy_penality()
{
	...
	call_virtual<void(__thiscall*)(void*)>(this, globals.indexes.at(11))(this);
}
...
bool weapon_t::can_fire(bool check_revolver)
{
	...
	auto server_time = TICKS_TO_TIME(globals.g.fixed_tickbase);
...
int weapon_t::get_max_tickbase_shift()
{
	...
	// you can actually control shift via lua(using convar), or even set convar in console  leftly
	if (!can_double_tap())
		return m_gamerules()->m_bIsValveDS() ? 6 : std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 12);
	...
	case WEAPON_NEGEV:
	case WEAPON_MP9:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 5);
		break;
	case WEAPON_ELITE:
	case WEAPON_UMP45:
	case WEAPON_BIZON:
	case WEAPON_TEC9:
	case WEAPON_MP7:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 6);
		break;
	case WEAPON_AK47:
	case WEAPON_AUG:
	case WEAPON_FAMAS:
	...
	case WEAPON_CZ75A:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 10);
		break;
	case WEAPON_FIVESEVEN:
	case WEAPON_GLOCK:
	case WEAPON_P250:
	case WEAPON_SG553:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 8);
		break;
	case WEAPON_HKP2000:
	case WEAPON_USP_SILENCER:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 9);
		break;
	case WEAPON_DEAGLE:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 12);
		break;
	case WEAPON_G3SG1:
	case WEAPON_SCAR20:
		max_tickbase_shift = std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 14);
		break;
	}

	if (m_gamerules()->m_bIsValveDS())
		max_tickbase_shift = min(max_tickbase_shift, std::clamp(sv_maxusrcmdprocessticks->GetInt(), 0, 6));
...
void player_t::modify_eye_position(Vector& eye_position)
{
	...
	auto head_position = Vector(globals.g.prediction_matrix[head_bone][0][3], globals.g.prediction_matrix[head_bone][1][3], globals.g.prediction_matrix[head_bone][2][3] + 1.7f);
...
int	player_t::get_move_type()
{
	...
	static auto m_MoveType = netvars::get().get_offset("CBaseEntity", "m_nRenderMode") + 1;
	if (m_MoveType)
...
void player_t::update_clientside_animation()
{
	...
	call_virtual<Fn>(this, globals.indexes.at(13))(this);
}
...
bool player_t::valid(bool check_team, bool check_dormant)
{
	...
	if (check_team && globals.local()->m_iTeamNum() == m_iTeamNum())
		return false;
...
CBaseHandle* player_t::m_hMyWeapons()
{
	return (CBaseHandle*)((uintptr_t)this + netvars::get().get_offset("CBaseCombatCharacter", "m_hActiveWeapon") - 256);
}

float& player_t::m_flSpawnTime()
{
	static auto m_iAddonBits = netvars::get().get_offset("CCSPlayer", "m_iAddonBits");
	return *(float*)(uintptr_t(this) + m_iAddonBits - 4);
}
c_baseplayeranimationstate* player_t::get_animation_state()
{
	static int m_bIsScoped = netvars::get().get_offset("CCSPlayer", "m_bIsScoped");
	return *reinterpret_cast<c_baseplayeranimationstate**>(reinterpret_cast<void*>(uintptr_t(this) + (m_bIsScoped - 0x14)));
}
...
bool player_t::setup_bones_rebuilt(matrix3x4_t* matrix, int mask)
{
	...
	*(uint8_t*)((uintptr_t)this + 0x274) = 0;
	...
	if (this != globals.local())
		set_abs_origin(m_vecOrigin());

	globals.g.setuping_bones = true;
	...
	globals.g.setuping_bones = false;
...
float_t player_t::get_max_desync_delta()
{
	if (!this) //-V704
		return 0.0f;
...
void viewmodel_t::SendViewModelMatchingSequence(int sequence)
{
	...
	call_virtual <Fn>(this, globals.indexes.at(14))(this, sequence);
}
...
void CHudChat::chat_print(const char* fmt, ...)
{
	...
	call_virtual <void(__cdecl*)(void*, int, int, const char*, ...)>(this, globals.indexes.at(15))(this, 0, 0, fmt);
	va_end(args);
}
