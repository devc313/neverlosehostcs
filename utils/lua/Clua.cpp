...
void lua_panic(sol::optional <std::string> message)
{
	...
	eventlogs::get().add(log, Color(255,255,255),false);
}
...
namespace ns_client
{
	...
	{		
		...
		{
			eventlogs::get().add(crypt_str("Lua error: invalid callback \"") + eventname + '\"', Color(255, 255, 255),false);
			return;
		}
	...
	{
		eventlogs::get().add(text, Color(255, 255, 255), false);
	}
}
...
namespace ns_menu
{
		...
		if (config_items.find(name) == config_items.end())
		{
			...
			for (auto item : cfg_manager->items)
			{
				if (item->name == name)
				{
					if (item->type != type)
					{
						eventlogs::get().add(crypt_str("Lua error: invalid config item type, must be ") + type, Color(255, 255, 255),false);
						return false;
					}
			...
			if (!found)
			{
				eventlogs::get().add(crypt_str("Lua error: cannot find config variable \"") + name + '\"', Color(255, 255, 255),false);
				return false;
			}
		}
	...
	bool get_bool(std::string name)
	{
		...
		if (it.type == NEXT_LINE)
		{
			...
			eventlogs::get().add(crypt_str("Lua error: cannot find menu variable \"") + name + '\"', Color(255, 255, 255),false);
			return false;
		}
...
namespace ns_globals
{
	int get_framerate()
	{
		return globals.g.framerate;
	}

	int get_ping()
	{
		return globals.g.ping;
	}
...
namespace ns_render
{
	...
	//void draw_ring_3D(float x, float y, float z, float r, float p, Color color, Color color2)
	//{
...
namespace ns_console
{
	...
	bool get_bool(const std::string& convar_name)
	{
		if (convars.find(convar_name) == convars.end())
		{
			...
			{
				eventlogs::get().add(crypt_str("Lua error: cannot find ConVar \"") + convar_name + '\"', Color(255, 255, 255), false);
				return false;
			}
		}
...
namespace ns_events
{
	...
	{
		if (std::find(globals.g.events.begin(), globals.g.events.end(), event_name) == globals.g.events.end())
		{
			m_eventmanager()->AddListener(&hooks::hooked_events, event_name.c_str(), false);
			globals.g.events.emplace_back(event_name);
		}
...
namespace ns_cmd
{
	...
	{
		if (!globals.get_command())
			return true;

		return globals.send_packet;
	}
	...
	bool get_button_state(int button)
	{
		...
		return globals.get_command()->m_buttons & button;
	}
	...
	void set_button_state(int button, bool state)
	{
		...
		if (state)
			globals.get_command()->m_buttons |= button;
		else
			globals.get_command()->m_buttons &= ~button;
	}
}
...
namespace ns_http //new
{
	std::string get(sol::this_state s, std::string& link)
	{
		if (!g_cfg.scripts.allow_http)
		{
			...
			eventlogs::get().add(crypt_str("Please, allow HTTP requests"), Color(255, 255, 255),false);
			return "";
		}
		...
		{
			eventlogs::get().add(crypt_str("Request failed, error: ") + std::string(e.what()), Color(255, 255, 255),false);
			return "";
		}
	}
...
namespace ns_file //new
{
	void append(sol::this_state s, std::string& path, std::string& data)
	{
		...
		{
			c_lua::get().unload_script(get_current_script_id(s));
			eventlogs::get().add(crypt_str("Please, allow files read or write"), Color(255, 255, 255), false);
			return;
		}
		...
		if (out.is_open())
			...
			eventlogs::get().add(crypt_str("Can't append to file: ") + path, Color(255, 255, 255), false);
	...
	void write(sol::this_state s, std::string& path, std::string& data)
	{
		...
		if (out.is_open())
			...
			eventlogs::get().add(crypt_str("Can't write to file: ") + path, Color(255, 255, 255), false);
	...
	std::string read(sol::this_state s, std::string& path)
	{
		...
		{
			eventlogs::get().add(crypt_str("Can't read file: ") + path, Color(255, 255, 255), false);
			file.close();
			return "";
		}
	}
