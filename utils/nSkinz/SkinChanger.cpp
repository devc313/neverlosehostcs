...
void updateHud() noexcept
{
	if (m_globals()->m_curtime >= last_skins_update)
	{
		...
		{
			if (auto hud_counts = *(hud_weapons + 0x20))
				for (auto i = 0; i < hud_counts; i++)
...
void SkinChanger::run(ClientFrameStage_t stage) noexcept
{
	...
	post_data_update_start(globals.local());
	...
	if (g_cfg.player.enable)
	{
		...
		switch (globals.local()->m_iTeamNum())
		{
		case 2:
		...
		if (player_model)
		{
			if (!globals.g.backup_model)
			{
				auto model = globals.local()->GetModel();
			...
			globals.local()->set_model_index(SkinChanger::player_model_indexes[player_model_index[player_model - 1]]);
			globals.g.backup_model = true;
		}
		else if (globals.g.backup_model)
		{
			globals.local()->set_model_index(backup_model_index);
	...
	/*if (UpdateRequired)
	{
		...
		globals.g.updating_skins = true;
	}
	else if (hudUpdateRequired && globals.g.updating_skins)
	{
...
void SkinChanger::scheduleHudUpdate() noexcept
{
	...
	if (m_globals()->m_curtime - last_skins_update < 1.0f)
		return;

	UpdateRequired = true;
	last_skins_update = m_globals()->m_curtime;
}
