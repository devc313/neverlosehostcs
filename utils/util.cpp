...
namespace util
{
	...
	std::string util::random_string(int length) {
	...
	void movement_fix(Vector& wish_angle, CUserCmd* m_pcmd)
	{
		if (m_pcmd->m_viewangles.z != 0.f
			&& !(globals.local()->m_fFlags() & FL_ONGROUND))
	...
	void copy_command(CUserCmd* cmd, int tickbase_shift)
	{
		...
		do
		{
			...
			auto verified_command = m_input()->GetVerifiedCmd(sequence_number);
